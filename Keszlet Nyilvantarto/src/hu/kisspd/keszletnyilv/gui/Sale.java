...
public class Sale extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jTextField_Item.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        jTextField_Item.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jTextField_ItemMouseClicked(evt);
            }
        });
        jTextField_Item.addKeyListener(new java.awt.event.KeyAdapter() {
            ...
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTextField_ItemKeyReleased(evt);
            }
        });
    ...
    private void jTextField_ItemMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTextField_ItemMouseClicked
        jTextField_Item.setText("");
        jTextField_Mennyiseg.setText("");
        jButton_Nyugta.setEnabled(false);
    }//GEN-LAST:event_jTextField_ItemMouseClicked
    ...
    private void jTextField_ItemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTextField_ItemKeyReleased
        searchTable();
    }//GEN-LAST:event_jTextField_ItemKeyReleased

    private void jTable_TermekMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable_TermekMouseClicked
        int line = jTable_Termek.getSelectedRow();
        cikkszam = jTable_Termek.getModel().getValueAt(line, 0).toString();
        itemName = jTable_Termek.getModel().getValueAt(line, 1).toString();
        stock = jTable_Termek.getModel().getValueAt(line, 2).toString();
        price = jTable_Termek.getModel().getValueAt(line, 3).toString();
        
        jTextField_Item.setText(itemName);
    ...
    private void jButton_NyugtaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_NyugtaActionPerformed
        ...
        String sql = "Select MAX(szamlaszam) as szlasz from Forgalom"; // el kell készíteni a forgalom táblát.
        ...
        if (!jTextField_Mennyiseg.getText().equals(""))
        {
            ...
            if ((dm.getRowCount() > 0))
            {
                while (validRequest && i < dm.getRowCount())
                {
                    if (dm.getValueAt(i, 2).equals(itemName))
                    {
                        newValue = Integer.parseInt(jTable_Szamla.getModel().getValueAt(i, 3).toString())+ Integer.parseInt(jTextField_Mennyiseg.getText());
                        Fizetendo = Integer.parseInt(price) * newValue;
                        validRequest = false;
                    }
                    if (validRequest)
                    {
                ...
                if (i >= dm.getRowCount())
                {
                    Fizetendo = Integer.parseInt(price) * Integer.parseInt(jTextField_Mennyiseg.getText());
                    dm.addRow(new Object[]{szlasz,cikkszam,itemName,jTextField_Mennyiseg.getText(),price,Login.userId,Fizetendo});
                    
                }
                else
                {
                    dm.setValueAt(newValue, i, 3);
                    dm.setValueAt(Fizetendo, i, 6);
                }
            }
            else
            {
                newValue=newValue + Integer.parseInt(jTextField_Mennyiseg.getText());
                Fizetendo = Integer.parseInt(price)* newValue;
                dm.addRow(new Object[]{szlasz,cikkszam,itemName,jTextField_Mennyiseg.getText(),price,Login.userId,Fizetendo});
            }
            jTextField_Item.setText("");
        ...
        {
            sum = sum + Integer.parseInt(jTable_Szamla.getModel().getValueAt(i, 6).toString());
        }
        jTextField_Osszeg.setText(Integer.toString(sum));
    ...
    private void jButton_FizetesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_FizetesActionPerformed
        ...
        try
        {
            ...
            for (int i = 0; i < jTable_Szamla.getRowCount(); i++)
            {
                conn = MySQLConn.ConnectDB();
                //TODO: replace old sqlQuery
                String sql = "INSERT INTO Forgalom (szamlaszam, T_cikkszam, F_Mennyiseg, Id) VALUES "
                        + "("+szlasz+","
        ...
        try
        {
            for (int i = 0; i < jTable_Szamla.getRowCount(); i++)
            {
                conn = MySQLConn.ConnectDB();
                //TODO: replace old sqlQuery
                String sql = "Select Keszlet from Termek where T_cikkszam="
                        + jTable_Szamla.getValueAt(i, 1);
                
                Statement stmi = conn.createStatement();
                ResultSet rsmi = stmi.executeQuery(sql);
                rsmi.next();
    ...
    private void searchTable()
    {
        ...
        try
        {
            ...
            String sql = "Select T_cikkszam, T_nev, Termek.Tcs_kod, Keszlet, T_ar "
                    + "FROM Termek WHERE T_nev LIKE '"+jTextField_Item.getText()+"%'";
            Statement sta = conn.createStatement();
            ResultSet rs = sta.executeQuery(sql);
